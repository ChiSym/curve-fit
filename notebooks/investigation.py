# %% [markdown]
# This notebook is used to work out importance and update computations outside of GenJAX.
# %%
...
xs = jnp.arange(4.0)
ys = jnp.array([0.1, 0.3, 0.9, 0.15])
cf = b.CurveFit(curve=quadratic, sigma_inlier=sigma_inlier, p_outlier=p_outlier)


# %% [markdown]
# Implementations of logpdf computations extracted from jaxprs.
# (These are not compactly written; the intent is to stay faithful
# to the jaxpr form)
# %%
...
def logpdf_uniform(v, low, high):
    ...
    j = high - low
    k = 1.0 / j
    m = jax.lax.select(i, 0.0, k)
    q = jax.lax.select(d, v, m)
    return jnp.log(q)
...
def importance_sample(cf, N, key):
    ...
    trs, ws = jax.vmap(cf.jitted_importance, in_axes=(0, None, None))(
        jax.random.split(k1, N), cmap, (xs,)
    )
...
def importance_score(tr):
    """The importance score of a curve is the sum of the logpdfs of the
    y coordinates of the fitted points based on the model, respecting
    ...
    def y_score(y, outlier, tr_y):
        return jax.lax.select(
            outlier, logpdf_uniform(y, -1.0, 1.0), logpdf_normal(tr_y, y, sigma_in)
        )
...
# %%
# Test an array of curves to show that shader importance and GenJAX importance agree.
...
def update_coefficient(tr: genjax.Trace, *, scale=0.01, key):
    """Compute update score for an update of the model coefficients."""
    ...
    our_w = jnp.sum(c1s - c0s) + jnp.sum(y_scores)
...
def update_inlier_sigma(tr, *, scale=0.01, key):
    "Compute update score if inlier sigma has changed."
    choices = tr.get_choices()
    k1, k2 = jax.random.split(key)
    ...
    our_w = logpdf_uniform(new_inlier_sigma, *sigma_inlier.args) - logpdf_uniform(
        old_inlier_sigma, *sigma_inlier.args
    )
    outliers = choices["ys", ..., "outlier"]
    ...
    def y_score(y, outlier, tr_y):
        return jax.lax.select(
            outlier,
            0.0,  # the target ys don't change, and that's what this part of the score is based
            logpdf_normal(tr_y, y, new_inlier_sigma)
            - logpdf_normal(tr_y, y, old_inlier_sigma),
        )
...
def update_p_outlier(tr, *, scale=0.01, key):
    """Compute update score for a a change to p_outlier"""
    choices = tr.get_choices()
    k1, k2 = jax.random.split(key)
